...
const firebaseServices = {
            ...
            try {
                ...
                await db.runTransaction(async (transaction) => {
                    ...
                    const newPayment = {
                        ...
                        timestamp: new Date().toISOString() // Usar fecha normal
                    };
    ...
    institutionalExpenses: {
        async addExpense(expenseData) {
            try {
                const docRef = await db.collection('institutionalExpenses').add({
                    ...expenseData,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp()
                });
                return { success: true, id: docRef.id };
            } catch (error) {
                console.error('Error agregando egreso institucional:', error);
                return { success: false, error: error.message };
            }
        },

        async getExpenses(limit = 50) {
            try {
                const snapshot = await db.collection('institutionalExpenses')
                    .orderBy('timestamp', 'desc')
                    .limit(limit)
                    .get();
                return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            } catch (error) {
                console.error('Error obteniendo egresos institucionales:', error);
                return [];
            }
        },
        ...
        async getTotalExpenses() {
            try {
                const snapshot = await db.collection('institutionalExpenses').get();
                let total = 0;
                ...
                console.error('Error calculando total de egresos:', error);
                return 0;
            }
        },

        // NUEVA FUNCIÓN: Eliminar egreso institucional
        async deleteExpense(expenseId) {
            try {
                await db.collection('institutionalExpenses').doc(expenseId).delete();
                return { success: true };
            } catch (error) {
                console.error('Error eliminando egreso institucional:', error);
                return { success: false, error: error.message };
            }
        }
    },
